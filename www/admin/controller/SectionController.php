 ...
 * Управление секциями
 *
 * `/admin/section/index` - список виджетов в секции
 * `/admin/section/up?id={widgetId}` - изменение порядка виджетов: поднять выше
 * `/admin/section/down?id={widgetId}` - изменение порядка виджетов: спустить ниже
 * `/admin/section/delete?id={widgetId}` - удаление виджета
 * `/admin/section/widget` - Создание или изменение виджета в секции
 *
 * @property-read string $section       (actionIndex)
 * @property-read Table  $table         (actionIndex)
 * @property-read array  $data          (actionWidget)
 * @property-read array  $type          (actionWidget)
 * @property-read array  $pageMenu      (actionWidget)
 * @property-read array  $userGroupList (actionWidget)
 * @property-read array  $pageOther     (actionWidget)
 */
class SectionController extends Controller {
	...
	public function actionIndex(): string {
		...
		if(is_array($items)===true) for($i=0,$cnt=count($items);$i<$cnt;$i++) {
	...
	/**
	 * Изменение порядка виджетов: поднять выше
	 */
	...
	/**
	 * Изменение порядка виджетов: спустить ниже
	 */
	...
	/**
	 * Создание или изменение виджета в секции
	 * @return string
	 */
	public function actionWidget(): string {
		$db=plushka::db();
		if(isset($_GET['id'])===true) { //Изменение
			...
			$this->data['type']=[$this->data['id'],$this->data['controller'],$this->data['action']]; //Нужен для загрузки (ajax) формы модуля.
			if($this->data['groupId']!==null) $this->data['groupId']=(int)$this->data['groupId'];
			...
			$this->data=['id'=>null,'groupId'=>null,'name'=>null,'title'=>'','cache'=>0,'data'=>null,'publicTitle'=>false,'cssClass'=>''];
		...
		$this->pageMenu=[0=>['title'=>null,'child'=>[],'parent'=>null]];
		while($item=$db->fetch()) $this->pageMenu[$item[0]]=['menuTitle'=>$item[2],'title'=>$item[3],'parent'=>$item[1],'child'=>[],'link'=>$item[4]];
		...
		array_unshift($this->userGroupList,['0','не авторизованные']);
	...
	protected function helpWidget(): string {
		return 'core/section#widgetNew';
	}

	public function actionWidgetSubmit(array $data): void {
		...
		$rule=[
			'id'=>['primary'],
			'groupId'=>['integer','группа пользователей','max'=>254],
			'section'=>['string'],
			'name'=>['latin','Имя',true],
			'data'=>['html'],
			'cache'=>['integer','Время кеширования'],
			'title'=>['string','Описание',true],
			'publicTitle'=>['boolean']
		];
		$user=plushka::user();
		if($user->group===255) $rule['cssClass']=['string'];
		if($model->save($rule)===false) return;
		...
		foreach($data['url'] as $link=>$item) {
			if(isset($item[1])===true && isset($item[2])===true) $link.='/';
			elseif(isset($item[1])===true) $link.='.';
			else $link.='*';
			$url[]=$link;
		}
		$db=plushka::db();
			...
			while($item=$db->fetch()) {
				...
				if($i!==false) unset($url[$i]);
					...
					if($s==='/') {
						...
						if(in_array(substr($item[0],0,$i1).'/',$url)===false) $delete[]=$item[0];
					}
				}
			}
			if($delete0) {
				...
				if($delete) {
					if(!plushka::hook('widgetPageDelete',$data['name'],$model->id,$delete)) return;
				}
	...
	private static function _pageOther(array $url,&$menu,int $level=0): array {
		static $title;
