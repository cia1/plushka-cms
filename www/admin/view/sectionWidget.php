<?php
use plushka\admin\controller\SectionController;
 ...
 * @return string
 */
function getHTMLMenu(array $data,int $level=0): string {
	$html='';
	foreach($data as $item) {
		if(isset($item['menuTitle'])===true) $html.='<b>'.$item['menuTitle'].'</b><br />';
...
<script>
    function loadForm(name, controller, action) {
        ...
        let url = '<?=plushka::url()?>admin/index2.php?controller=' + controller + '&action=' + action<?php if(isset($_GET['_front'])) echo '+"&_front"'; ?>+
            "&section=<?=$this->data['section']?>&_serialize";
        jQuery('#_type').load(url, {'data': '<?=$this->data['data']?>'}, function () {
            let f = $('#_type form');
            let action = f.attr('action') + '&_serialize';
...
<cite>В блоке <b>Страницы</b> отметье те страницы, на которых должен публиковаться виджет.<br/>
    Вложенные страницы - страницы, находящиеся "глубже" указанной, например если есть такая структура: &laquo;Главная &rarr; Новости &rarr; Первая новость&raquo;,
    то "Первая новость" является вложенной по отношению к "Новости". Если у пункта "Новости" отмечено "на этой странице" и не отмечено "на вложенных страницах",
    то виджет будет публиковаться в разделе "Новости", но не будет на странице конкретной новости.<br/>
    В поле <b>другие URL</b> впишите страницы (через запятую), на которых должен публиковаться виджет, но которых нет в списке выше. В конце каждой страницы
    нужно поставить символ &laquo;.&raquo; (только на текущей странице), &laquo;*&raquo; (только на вложенных страницах) или &laquo;/&raquo; (на текущей и
    вложенных страницах). Например: "shop/cat1/,article/view/rules.,some/link/3*"</cite>