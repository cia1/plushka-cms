...
$f->render();
?>
<hr style="clear:both;width:90%;margin:0 auto 10px auto;border:none;border-bottom:1px solid #eee;"/>
...
<script>
    function loadForm(id, controller, action) {
        ...
        let url = '<?=plushka::url()?>admin/index2.php?controller=' + controller + '&action=' + action + '&_lang=<?=_LANG?>'<?php if(isset($_GET['_front']))
					echo '+"&_front"'; ?>+ "&link=<?=urlencode($this->data['link'])?>";
        $('#_type').load(url, function () { //После загрузки формы модуля
            setTimeout(function () {
                let f = $('#_type form');
