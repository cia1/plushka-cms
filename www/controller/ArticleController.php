...
class ArticleController extends Controller {
	...
	protected function breadcrumbView(): array {
		if($_GET['corePath'][1]==='view') return ['{{pageTitle}}']; else return [
			'<a href="'.plushka::link('article/'.$_GET['corePath'][1].'/'.$this->category['alias']).'">'.$this->category['title'].'</a>',
			'{{pageTitle}}'
		];
	}

	protected function adminViewLink(): array {
		if($_GET['corePath'][1]==='list') $s='&list'; elseif($_GET['corePath'][1]==='blog') $s='&blog';
		else $s='';
		return [
			[
				'article.article',
				'?controller=article&action=article&id='.$this->article['id'].$s,
				'edit',
				'Редактировать статью'
			],
			[
				'article.article',
				'?controller=article&action=articleDelete&id='.$this->article['id'],
				'delete',
				'Удалить статью &laquo;'.$this->article['title'].'&raquo;',
				'Удалить',
	...
	protected function breadcrumbBlog(): array {
		return ['{{pageTitle}}'];
	}

	protected function adminBlogLink(): array {
		return [
			[
				'article.category',
				'?controller=article&action=category&id='.$this->category['id'],
				'edit',
				'Править заголовок, мета-теги блога'
			],
			[
				'article.article',
				'?controller=article&action=article&categoryId='.$this->category['id'].'&blog',
				'new',
				'Добавить новую статью в блог'
			],
			[
				'article.article',
				'?controller=article&action=feature&categoryId='.$this->category['id'].'&blog',
				'status0',
	...
	protected function adminBlogLink2(array $data): array {
		return [
			[
				'article.article',
				'?controller=article&action=article&id='.$data['id'].'&blog',
				'edit',
				'Редактировать статью &laquo;'.$data['title'].'&raquo;'
			],
			[
				'article.article',
				'?controller=article&action=articleDelete&id='.$data['id'],
				'delete',
				'Удалить статью &laquo;'.$data['title'].'&raquo;',
				'Удалить',
				'if(!confirm(\'Подтвердите удаление.\')) return false;'
			]
		];
	...
	protected function adminListLink(): array {
		return [
			[
				'article.category',
				'?controller=article&action=category&id='.$this->category['id'],
				'edit',
				'Править заголовок, мета-теги, описание'
			],
			[
				'article.article',
				'?controller=article&action=article&categoryId='.$this->category['id'].'&list',
				'new',
				'Добавить новую статью в категорию'
			],
			[
				'article.article',
				'?controller=article&action=feature&categoryId='.$this->category['id'].'&list',
				'status0',
				'Не опубликованные статьи'
			]
		];
