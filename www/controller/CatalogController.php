...
*/
class CatalogController extends \plushka\core\Controller {
	...
	public function actionIndex() {
		...
		$f=plushka::path().'data/catalog/'.$this->layoutId.'.html';
		...
		foreach($_GET as $key=>$value) {
			...
			if($key!=='title' && $layout[$key]['type']=='integer') $value=explode('-',$value);
			if(is_array($value)===false) {
				...
				if($where) $where.=' AND ';
				if($v1>0 && $v2>0) $where.=$key.' BETWEEN '.$v1.' AND '.$v2;
				elseif($v1>0) $where.=$key.'>'.$v1;
				elseif($v2>0) $where.=$key.'<'.$v2;
			}
		}
		...
		while($item=$db->fetchAssoc()) {
			...
			foreach($ids as $id) {
				if($layout[$id]['type']=='image') {
					$v=plushka::url().'public/catalog/'.$item[$id];
				} else $v=$item[$id];
				$data['field'][$id]=array('value'=>$v,'layout'=>$layout[$id]);
			}
			if($isLink) $data['link']=plushka::link('catalog/'.$this->layoutId.'/'.$item['alias']); else $data['link']=null;
			$this->data[]=$data;
		}
		unset($this->layout);
	...
	public function actionView() {
		...
		foreach($view as $field) { //перебрать все поля
			...
			$field['value']=$data[$id];
			if($field['type']=='image') $field['value']=plushka::url().'public/catalog/'.$field['value'];
			elseif($field['type']=='gallery') {
				...
				if($v) {
					$v=explode('|',$v);
					for($i=0,$cnt=count($v);$i<$cnt;$i++) $v[$i]=plushka::url().'public/catalog/'.$v[$i];
				} else $v=array();
			}
			$this->data[$id]=$field;
		}
	...
	public function breadcrumbView() {
		return array('<a href="'.plushka::link('catalog/'.$this->layoutId).'">'.LNGCatalog.'</a>','{{pageTitle}}');
	}
