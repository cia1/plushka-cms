...
class FaqController extends \plushka\core\Controller {
	...
	public function actionIndexSubmit($data) {
		if($_SERVER['SCRIPT_NAME']==plushka::url().'index.php') $inFrame==false; else $inFrame=true;
		$m=plushka::model('faq');
		$data['date']=time();
		$m->set($data);
		if(!$m->save(array(
		...
		if(!$inFrame) plushka::redirect('faq',LNGThankyouForQuestion);
		plushka::success(LNGThankyouForQuestion);
