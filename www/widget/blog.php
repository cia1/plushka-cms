...
class widgetBlog extends widget {
	...
	public function __invoke() {
		...
		$this->categoryAlias=$db->fetchValue('SELECT alias FROM articleCategory WHERE id='.$this->options['categoryId']);
		$db->query('SELECT id,alias,date,title,text1 FROM article WHERE categoryId='.$this->options['categoryId'].' AND date<'.time().' ORDER BY sort,date DESC LIMIT 0,'.$countTotal);
