...
for($i=0,$cnt=count($items);$i<$cnt;$i++) {
	...
	echo '<url><loc>//'.$_SERVER['HTTP_HOST'].plushka::link($item[0]).'</loc>'.($m ? '<lastmod>'.date('Y-m-d',$m).'</lastmod>' : '').'</url>'."\n";
}
echo "\n";
...
for($i=0,$cnt=count($items);$i<$cnt;$i++) {
	...
	echo '<loc>//'.$_SERVER['HTTP_HOST'].plushka::link($l).'</loc>'.($m ? '<lastmod>'.date('Y-m-d',$m).'</lastmod>' : '');
	echo "</url>\n";
}